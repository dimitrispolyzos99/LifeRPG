    ...
    var body: some View {
        VStack(spacing: 10){
            ...
            Text(viewModel.player.playerClass.name)
                .font(.headline)
                .bold()
            Text("HP \(viewModel.player.hp)/\(viewModel.maxPlayerHP)")
                .font(.caption)
            ProgressView(value: Double(viewModel.player.hp), total: Double(viewModel.maxPlayerHP))
                    .tint(.red)
                    .padding()
                    .frame(height: 6)
            Text("MP \(viewModel.player.mana)/\(viewModel.maxPlayerMana)")
                .font(.caption)
            ProgressView(value: Double(viewModel.player.mana), total: Double(viewModel.maxPlayerMana))
                    .tint(.blue)
                    .padding()
                    .frame(height: 9)
